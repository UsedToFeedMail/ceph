...
bool MDCache::process_imported_caps()
{
  ...
  if (rejoin_gather.count(mds->get_nodeid()) == 0) {
	...
	for (map<int,ceph_mds_cap_reconnect>::iterator r = q->second.begin();
	  ...
	  Capability *cap = rejoin_import_cap(in, q->first, r->second, r->first);
	  if (r->first >= 0) {
...
void MDCache::handle_cache_expire(MCacheExpire *m)
{
  ...
  for (map<dirfrag_t,MCacheExpire::realm>::iterator p = m->realms.begin();
    ...
    // INODES
    for (map<vinodeno_t,__u32>::iterator it = p->second.inodes.begin();
    ...
    // DIRS
    for (map<dirfrag_t,__u32>::iterator it = p->second.dirs.begin();
    ...
    // DENTRIES
    for (map<dirfrag_t, map<pair<string,snapid_t>,__u32> >::iterator pd = p->second.dentries.begin();
	...
	assert(dir->is_auth());
      ...
      for (map<pair<string,snapid_t>,__u32>::iterator p = pd->second.begin();
	   p != pd->second.end();
	   ++p) {
